...
public class RecastDemo {
    ...
    public void start() {
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            else if(settingsUI.isSaveTriggered()){
                if(sample.getNavMesh() != null){
                    JFileChooser chooser = new JFileChooser();

                    chooser.setCurrentDirectory(new File("F:\\WorkSpaces\\Java\\Game\\android\\assets\\navmeshs"));
                    int returnVal = chooser.showOpenDialog(null);
                    if(returnVal == JFileChooser.APPROVE_OPTION) {
                        System.out.println("You chose to open this file: " +
                                chooser.getSelectedFile().getName());
                        NavMesh navMesh1 = sample.getNavMesh();
                        try {
                            UnityAStarPathfindingImporter.saveMesh(navMesh1, chooser.getSelectedFile());
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                    }
